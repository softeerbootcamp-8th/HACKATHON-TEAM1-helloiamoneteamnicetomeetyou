...
export function BottomSheet({ open, onOpenChange, peek, height, header, children }: Props) {
  ...
  return (
    <motion.div
      ...
      className="absolute inset-x-0 bottom-0 z-30 flex touch-none flex-col rounded-t-[26px] bg-white shadow-[0_-8px_30px_rgba(0,0,0,0.10)]"
    >
      <button
        type="button"
        onClick={() => onOpenChange(!open)}
        aria-expanded={open}
        className="shrink-0 cursor-grab px-5 pt-2.5 pb-3 active:cursor-grabbing"
      >
