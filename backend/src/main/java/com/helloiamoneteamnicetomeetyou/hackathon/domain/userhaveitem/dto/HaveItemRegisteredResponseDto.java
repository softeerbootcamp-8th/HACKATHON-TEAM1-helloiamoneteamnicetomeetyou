 ...
 * @param quantity   지금 새로 내놓을 수 있는 개수 ({@code quantityLeft}). 총 등록 수량이 아니다.
 *                   이미 다른 교환에 예약되었거나 다 나간 만큼은 빠져 있다
 * @param registered 내놓기로 등록해 둔 개수. 교환에 묶인 몫까지 합친 값이다. 등록 화면이 되살릴
 *                   값은 이쪽이다. {@code quantity} 로 되살리면 통째로 묶인 카드가 화면에서
 *                   사라지고, 교환으로 받아 두기만 한 카드가 내놓은 것처럼 나온다
 * @param reserved   교환에 예약되어 해제할 수 없는 카드인가
 */
public record HaveItemRegisteredResponseDto(
        Long itemId, Integer quantity, Integer registered, boolean reserved) {

    public static HaveItemRegisteredResponseDto from(UserHaveItem haveItem) {
        return new HaveItemRegisteredResponseDto(
                haveItem.getItem().getId(),
                haveItem.getQuantityLeft(),
                haveItem.getRegisteredQuantity(),
                haveItem.isReserved());
    }
}
