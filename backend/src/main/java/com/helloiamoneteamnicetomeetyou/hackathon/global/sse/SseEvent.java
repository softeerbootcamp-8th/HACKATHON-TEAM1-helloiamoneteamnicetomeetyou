 ...
 *
 * <p>{@code zoneId} 와 {@code userId} 중 채워진 쪽이 전송 대상을 정한다. 둘을 함께 받는 대신
 * 정적 팩터리 두 개로 갈라 둬서, 부르는 쪽에서 대상을 헷갈릴 여지를 없앴다.
 */
public record SseEvent(SseEventType type, Object data, Long zoneId, UUID userId) {

    public static SseEvent toZone(Long zoneId, SseEventType type, Object data) {
        return new SseEvent(type, data, zoneId, null);
    }
